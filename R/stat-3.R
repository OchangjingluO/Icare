...
#' @param palette_name A string specifying the color palette to be used for the heatmap (default is "beyonce::X6").
...
highly_correlated_features <- function(data,
  ...
  upper <- corr_matrix[upper.tri(corr_matrix)]
  to_drop <- names(which(upper > correlation_threshold))
  correlated_features <- unique(to_drop)

  cat("Identified correlated features:\n", paste(correlated_features, collapse = ", "), "\n")
  ...
  for (column in correlated_features) {
    corr_features <- names(which(corr_matrix[, column] > correlation_threshold))
    if (length(corr_features) > 0) {
      corr_values <- corr_matrix[corr_features, column]
      drop_features <- rep(column, length(corr_features))
      temp_df <- data.frame(drop_feature = drop_features, corr_feature = corr_features, corr_value = corr_values)
  ...
  plot_clustermap_heatmap <- function(corr_matrix, palette_name, title) {
    ...
    color_palette <- colorRampPalette(colors[2:6])(100)

    heatmap(as.matrix(corr_matrix),
            trace = "none",
            margins = c(10, 10),
            col = color_palette,
            key = TRUE,
    ...
    text(x = 1:ncol(corr_matrix), y = par("usr")[4] + 0.5, labels = colnames(corr_matrix), font = 2, xpd = TRUE)
    text(y = 1:nrow(corr_matrix), x = par("usr")[1] - 0.5, labels = rownames(corr_matrix), font = 2, xpd = TRUE)
  }

  if (save_plots) {
    dir.create(save_dir, showWarnings = FALSE, recursive = TRUE)
    pdf(file.path(save_dir, "correlation_heatmap.pdf"), width = 10, height = 8)
    plot_clustermap_heatmap(corr_matrix, palette_name, heatmap_title)
    dev.off()
  ...
  result <- list(corr_matrix = corr_matrix,
...
#' @param palette_name A string specifying the color palette for the plot (default is "beyonce::X6").
...
plot_top_correlations <- function(data,
  ...
  if (top_n > n_features) {
    warning(paste("top_n is greater than the number of features in the data:", n_features, "Setting top_n to", n_features))
    top_n <- n_features
  }

  if (!is.null(group_col) && group_col %in% colnames(data)) {
    data <- data[, !colnames(data) %in% group_col]
  }
  ...
  corrplot(top_cor_matrix, order = 'AOE', col = colors, tl.col = "black", tl.cex = base_size / 14, plotCI = NULL)
...
#' @param palette_name A string specifying the color palette to be used for the plot (default is "beyonce::X6").
#' @param save_plots A logical value indicating whether to save the plot as a PNG file (default is TRUE).
...
#'          If `save_plots` is TRUE, the plot is saved as a PNG file.
#'
#' @export
#'
#' @examples
...
cross_analysis <- function(data,
  ...
  var1 <- vars[1]
  var2 <- vars[2]
  colors <- paletteer::paletteer_d(palette_name)
  ...
  if (is.numeric(data[[var1]]) && is.numeric(data[[var2]])) {
    ...
    if (save_plots) {
      ggsave(filename = file.path(save_dir, paste0("scatter_plot_", var1, "_vs_", var2, ".png")),
             plot = gg,
             width = plot_width, height = plot_height, dpi = 300)
      cat("Plot saved to:", file.path(save_dir, paste0("scatter_plot_", var1, "_vs_", var2, ".png")), "\n")
    }
...
#' The plot will be saved as a PNG file if specified.
#'
