...
#' saved to a specified directory as a PNG file.
#'
...
#' @param palette_name A string indicating the name of the color palette to use for the chart (default: "beyonce::X6").
...
#' plot_deg_radarchart(df, palette_name = "beyonce::X6", title = "Differential Expression Radar")
plot_deg_radarchart <- function(df,
  ...
  ggsave(filename = file.path(save_dir, "radar_chart.png"), plot = plot, width = plot_width, height = plot_height)
  cat("Radar chart saved to: ", file.path(save_dir, "radar_chart.png"))
...
#' @param palette_name A string indicating the color palette for the plot (default: "beyonce::X6").
#' @param plot_width The width of the plot (default: 5).
#' @param plot_height The height of the plot (default: 5).
...
#' VarFeature_radarchart(stat_object, palette_name = "beyonce::X6", title = "Variable Feature Radar")
#'
#' # Example usage with a data frame:
#' VarFeature_radarchart(df, palette_name = "beyonce::X6", title = "Variable Feature Radar")
...
#' @param palette_name A string indicating the color palette to use for the plot (default: "beyonce::X6").
...
#'
#' plot_deg_volcano(last_test_sig = deg_results, title = "Custom Volcano Plot", palette_name = "beyonce::X6")
...
plot_deg_volcano <- function(last_test_sig,
  ...
  colors <- as.vector(paletteer::paletteer_d(palette_name))
  significant_color <- colors[3]
  not_significant_color <- colors[4]
  ...
  ggsave(filename = file.path(save_dir, "volcano_plot.png"), plot = volcano_plot,
         width = plot_width, height = plot_height)

  cat("Volcano plot saved to: ", file.path(save_dir, "volcano_plot.png"), "\n")
...
#'
#' VarFeature_volcano(object = stat_object, title = "Custom Volcano Plot", palette_name = "beyonce::X6")
...
#' @param palette_name The name of the color palette for the plot (default: "beyonce::X6").
...
plot_deg_violinplot <- function(last_test_sig,
  ...
  pal <- as.vector(paletteer::paletteer_d(palette_name)[3:6])
  ...
  p <- ggplot(data = box_test, aes(x = id, y = log10(value), fill = .data[[group_col]])) +
    ...
    ggforce::geom_sina(color = '#cccccc', size = 0.3) +
  ...
  if (save_plots) {
    ...
    output_path <- file.path(save_dir, "violinplot.png")
    ggsave(filename = output_path, plot = p, height = plot_height, width = plot_width, units = 'cm')
...
#' @param palette_name The color palette to use for the plot (default is "beyonce::X6").
...
#' @param palette_name The name of the palette to be used for the ROC curves. Default is `"beyonce::X6"`.
...
plot_deg_Roc_plot <- function(deg_test,
  ...
  colors <- paletteer::paletteer_d(palette_name, n = length(roc_tem))
  p <- ggroc(roc_tem) +
    theme_classic() +
  ...
  if (save_plots) {
    ...
    output_path <- file.path(save_dir, "roc_plot.png")
    ggsave(filename = output_path, plot = p, height = plot_height, width = plot_width, units = 'cm')
  }
